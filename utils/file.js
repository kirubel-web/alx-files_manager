...
const fileUtils = {
  ...
  /**
   * Validates if body is valid for creating a file
   ...
   * @fileParams {object} object with attributes of file to save
   ...
   * @query {object} query used to find file
   * @return {object} file
   */
  async getFile(query) {
  ...
  async getFilesOfParentId(query) {
    return dbClient.filesCollection.aggregate(query).toArray();
  },

  /**
   * Updates a file document in the database
   * @query {object} query to find document to update
   * @set {object} object with query info to update in Mongo
   * @return {object} updated file
   */
  async updateFile(query, set) {
    return dbClient.filesCollection.findOneAndUpdate(query, set, {
